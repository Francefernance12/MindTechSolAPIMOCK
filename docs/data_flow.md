...
- The user will load into the frontend.
- Then react will start fetching data from the backend
- By sending a HTTP GET request(or URL) along with the API Key in the headers
- Once Flask receives that request, it would start processing, and validating it
- By checking the API Key first, and check if the user is authorized to access the DB or not.
- If validated, Flask will start quiering the DB.
- The DB would return the rows to flask.
- Flask would start turning those rows into lists of dictionaries
- So they can be formatted into JSON
- Then that JSON array would be returned to the frontend.
- This allows react to use the data in the frontend.
- For example, each client card has data that was sent from the backend and database.
